	document.addEventListener('DOMContentLoaded', () => {
		...
		toggleButton.addEventListener('click', () => {
			...
			if (result.mode === 'popup') {
				console.log('Le mode est déjà en mode popup, pas de changement nécessaire.');
			} else {
				// Mettre à jour le mode en "popup"
				...
				chrome.runtime.sendMessage({ action: 'closeSidePanel' }, (response) => {
					...
					console.error('Erreur lors de la fermeture du panneau latéral:', chrome.runtime.lastError);
					} else {
					console.log('Réponse du Service Worker:', response.status);
					}
				});
				});
			}
